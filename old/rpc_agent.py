...
class FreelanceClient(object):
    ...
    def __init__(self):
        ...
        self.agent = threading.Thread(target=do_agent_task, args=(self.context, agent_command_socket))
    ...
    def request(self, requester, msg):
        requester.send_multipart([b"REQUEST"] + msg, flags=zmq.NOBLOCK)

    def receive(self, requester):
        return requester.recv_multipart(flags=zmq.NOBLOCK)
...
def do_agent_task(ctx, command_socket):
    ...
    poll_backend.register(agent.command_socket, zmq.POLLIN)
            ...
            for request in list(agent.requests.values()):
                if now >= request.expires:
                    if request.retry(now):
                        print("I: RETRYING REQUEST  %s, remaining %d" % (request.sequence, request.left_retries))
