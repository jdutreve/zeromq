...
class FreelanceClient(object):
    ...
    def __init__(self):
        ...
        self.commands, agent_command_socket = zpipe(self.context, agent_context, zmq.PAIR, zmq.PAIR)
        self.requests, agent_request_socket = zpipe(self.context, agent_context, zmq.DEALER, zmq.ROUTER, int(CLIENT_QUEUE_SIZE / 2))
        ...
        self.agent = threading.Thread(target=agent_task, args=(agent_context, agent_command_socket, agent_request_socket))
    ...
    async def request(self, msg):
        "Send request"
        await self.requests.send_multipart([b"REQUEST"] + msg)
